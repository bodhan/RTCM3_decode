...
class RTCM3:
    def __init__(self):
        self.buffer1 = bytearray(''*10000)
        self.buffer2 = bytearray(''*10000)
        self.length = 0
        self.length2 = 0
        self.msg_id = 0
        self.msg_id2 = 0
    ...
    def decode(self, data, length, msg_id):
        computed_crc = crc_normal(data[0:length+3])
        crc = data[3+length]
        crc = crc << 8
...
def decode_1077(data):
    ...
    bit_array = makeBitArray(data)
    current_bit = 12
    msg_1077.reference_station_id = bitValue(bit_array, current_bit, 12)
    current_bit += 12
    msg_1077.gps_epoch_time = bitValue(bit_array, current_bit, 30)*0.001
    current_bit += 30
    msg_1077.multiple_message_bit = bitValue(bit_array, current_bit, 1)
    current_bit += 1
    msg_1077.idos = bitValue(bit_array, current_bit, 3)
    current_bit += 3
    current_bit += 7
    msg_1077.clock_steering_indicator = bitValue(bit_array, current_bit, 2)
    current_bit += 2
    msg_1077.external_clock_indicator = bitValue(bit_array, current_bit, 2)
    current_bit += 2
    msg_1077.gps_dsi = bitValue(bit_array, current_bit, 1)
    current_bit += 1
    msg_1077.gps_si = bitValue(bit_array, current_bit, 3)
    current_bit += 3
    for i in range(1,65):
        mask = bitValue(bit_array, current_bit, 1)
        current_bit += 1
        if mask:
    ...
    for i in range(0, msg_1077.nsat * msg_1077.nsig):
        msg_1077.cellmask[i] = bitValue(bit_array, current_bit, 1)
        current_bit += 1
